<script lang="ts">
	...
	// When true, render params inside collapsible sub-categories (used by the model wizard).
	// Default false keeps the flat list used elsewhere (e.g. chat settings) unchanged.
	export let grouped = false;
	let openGroups = { gen: true, sampling: true, penalties: false, runtime: false };
...
<div class=" space-y-1 text-xs pb-safe-bottom">
	...
	{#if grouped}
		<button
			type="button"
			class="apg-group"
			on:click={() => (openGroups.sampling = !openGroups.sampling)}
		>
			<span class="apg-chevron {openGroups.sampling ? 'open' : ''}">⌄</span>
			{$i18n.t('Sampling & Length')}
		</button>
	{/if}
	{#if !grouped || openGroups.sampling}
		<div class="space-y-1">
			<div class=" py-0.5 w-full justify-between">
				<Tooltip
					content={$i18n.t(
						...
						<button
							...
							on:click={() => {
								params.reasoning_effort =
									(params?.reasoning_effort ?? null) === null ? 'medium' : null;
							}}
						>
	...
	{#if grouped}
		<button
			type="button"
			class="apg-group"
			on:click={() => (openGroups.penalties = !openGroups.penalties)}
		>
			<span class="apg-chevron {openGroups.penalties ? 'open' : ''}">⌄</span>
			{$i18n.t('Penalties & Mirostat')}
		</button>
	{/if}
	{#if !grouped || openGroups.penalties}
		<div class="space-y-1">
			<div class=" py-0.5 w-full justify-between">
				<Tooltip
					content={$i18n.t(
						...
						<button
							...
							on:click={() => {
								params.frequency_penalty =
									(params?.frequency_penalty ?? null) === null ? 1.1 : null;
							}}
						>
	...
	{#if grouped}
		<button
			type="button"
			class="apg-group"
			on:click={() => (openGroups.runtime = !openGroups.runtime)}
		>
			<span class="apg-chevron {openGroups.runtime ? 'open' : ''}">⌄</span>
			{$i18n.t('Runtime & Ollama')}
		</button>
	{/if}
	{#if !grouped || openGroups.runtime}
		<div class="space-y-1">
			{#if admin}
				<div class=" py-0.5 w-full justify-between">
					<Tooltip
...
<style>
	.apg-group {
		...
		font-size: 12px;
		font-weight: 600;
		color: var(--text-secondary, #6b6964);
		border-top: 1px solid var(--border, rgba(0, 0, 0, 0.06));
	}
	.apg-group:hover {
		color: var(--text, #2a2927);
	}
	.apg-chevron {
