const mapData = {
	...
	nodes: [
		node("root-layout", "Root layout", "screen", 160, 1120, "src/routes/+layout.svelte", "Initializes config, user session, theme, websocket, i18n, Pyodide worker, app sidebar, sync modal, and frontend update handling."),
		node("auth", "Auth", "screen", 160, 1520, "src/routes/auth/+page.svelte", "Login and account access route outside the authenticated app shell.", true),
		node("app-layout", "Authenticated app layout", "screen", 520, 1120, "src/routes/(app)/+layout.svelte", "Guards authenticated users, loads settings/models/tools/banners, checks local chat DB, and hosts sidebar/settings/changelog overlays."),
		node("sidebar", "Global sidebar", "component", 880, 650, "src/lib/components/layout/Sidebar.svelte", "Chat list, folders, pinned chats/notes/models, channels, pinned menu links, user menu, search, import/export, and sidebar width state."),
		node("navbar-menu", "Navbar menu", "component", 880, 1010, "src/lib/components/layout/Navbar/Menu.svelte", "Shared navbar actions and menu surface used around app screens."),
		node("search-modal", "Search modal", "component", 880, 1370, "src/lib/components/layout/SearchModal.svelte", "Global search entry point driven by showSearch state."),
		node("settings-modal", "User settings modal", "component", 880, 1730, "src/lib/components/chat/SettingsModal.svelte", "User settings tabs for account, interface, personalization, audio, connections, integrations, data controls, advanced params, tools, and about."),
		node("stores", "Svelte stores", "store", 520, 520, "src/lib/stores/index.ts", "Central writable state for user/config/settings/models/chats/sidebar/modal visibility/socket/channels/tools/functions/file nav/artifacts."),
		node("chat-home", "Chat home", "screen", 1240, 980, "src/routes/(app)/+page.svelte", "Default authenticated route. Instantiates Chat with no chat id and handles URL error toast.", true),
		node("chat-existing", "Chat detail", "screen", 1240, 1260, "src/routes/(app)/c/[id]/+page.svelte", "Instantiates Chat with chatIdProp from route params for an existing conversation.", true),
		node("shared-chat", "Shared chat", "screen", 1240, 1540, "src/routes/s/[id]/+page.svelte", "Public/shared chat view outside the authenticated app route group."),
		node("watch", "Watch", "screen", 1240, 1800, "src/routes/watch/+page.svelte", "Standalone watch route."),
		node("chat", "Chat component", "component", 1600, 1120, "src/lib/components/chat/Chat.svelte", "Main conversation orchestration: model selection, history tree, file/web uploads, streaming, task lifecycle, socket events, artifacts, file nav, and persistence."),
		node("chat-navbar", "Chat navbar", "component", 1960, 690, "src/lib/components/chat/Navbar.svelte", "Conversation header and controls around sidebar toggle, title, share/tag/actions, and chat context."),
		node("model-selector", "Model selector", "component", 1960, 950, "src/lib/components/chat/ModelSelector.svelte", "Selects one or more models, pins defaults, and updates user settings."),
		node("message-input", "Message input", "component", 1960, 1210, "src/lib/components/chat/MessageInput.svelte", "Prompt editor, slash commands, variable expansion, files, web/vision/audio/screen capture, terminal menu, integrations, and submit flow."),
		node("messages", "Messages", "component", 1960, 1490, "src/lib/components/chat/Messages.svelte", "Builds the visible message branch, pagination, edit/regenerate/delete/rating/actions, and update persistence."),
		node("chat-controls", "Chat controls", "component", 1960, 1770, "src/lib/components/chat/ChatControls.svelte", "Side control pane for selected tools, filters, generation params, embeds, and valves."),
		node("file-nav", "File nav / terminal files", "component", 1960, 2050, "src/lib/components/chat/FileNav.svelte", "Browse, preview, edit, bulk operate, and inspect terminal-backed files and ports."),
		node("chat-functions", "Chat major functions", "function", 2320, 1120, "src/lib/components/chat/Chat.svelte", "navigateHandler, initNewChat, loadChat, createMessagePair, submitPrompt, processNextInQueue, chatCompletedHandler, chatActionHandler, uploadWeb, uploadGoogleDriveFile, terminalEventHandler."),
		node("input-functions", "Input major functions", "function", 2320, 1470, "src/lib/components/chat/MessageInput.svelte", "inputVariableHandler, replaceVariables, getCommand, uploadFileHandler, inputFilesHandler, screenCaptureHandler, createNote, keyboard and paste handlers."),
		node("message-functions", "Message major functions", "function", 2320, 1820, "src/lib/components/chat/Messages.svelte", "buildMessages, loadMoreMessages, editMessage, saveMessage, deleteMessage, rateMessage, showPreviousMessage, showNextMessage."),
		node("api-core", "Core API barrel", "api", 2680, 760, "src/lib/apis/index.ts", "Shared backend helpers including getBackendConfig, getModels, generateQueries, chatAction, MoA completion, version, and tool server data."),
		node("api-chats", "Chats API", "api", 2680, 1040, "src/lib/apis/chats/index.ts", "CRUD, list/search, archive, share, clone, pin, tags, folders, access grants, and chat stats exports."),
		node("api-openai", "OpenAI API client", "api", 2680, 1320, "src/lib/apis/openai/index.ts", "OpenAI-compatible config, URLs, keys, model discovery, connection verification, chat completion, and speech synthesis."),
		node("api-retrieval", "Retrieval API", "api", 2680, 1600, "src/lib/apis/retrieval/index.ts", "RAG/query/embedding/rerank configs plus processWeb, processWebSearch, processYoutubeVideo, queryDoc, and queryCollection."),
		node("api-files", "Files API", "api", 2680, 1880, "src/lib/apis/files/index.ts", "File upload, process status, list/search, content retrieval, content update, and deletion."),
		node("api-tasks", "Tasks / streaming APIs", "api", 2680, 2160, "src/lib/apis/tasks/index.ts + streaming", "Active chat checks, task ids/stop handling, and text stream creation."),
		node("workspace-layout", "Workspace layout", "screen", 1240, 240, "src/routes/(app)/workspace/+layout.svelte", "Permission-gated workspace shell with tab navigation for models, knowledge, prompts, skills, and tools."),
		node("workspace", "Workspace index", "screen", 1600, 130, "src/routes/(app)/workspace/+page.svelte", "Workspace landing/redirect surface."),
		node("workspace-models", "Workspace models", "screen", 1600, 350, "src/routes/(app)/workspace/models/+page.svelte", "Model inventory screen using Models component for search, import/export, pin, visibility, enablement, clone, share, and delete.", true),
		node("model-editor", "Model editor", "component", 1960, 250, "src/lib/components/workspace/Models/ModelEditor.svelte", "Create/edit model metadata, prompt suggestions, knowledge, tools, skills, filters, capabilities, terminal, and access."),
		node("workspace-knowledge", "Workspace knowledge", "screen", 1600, 570, "src/routes/(app)/workspace/knowledge/+page.svelte", "Knowledge base list with search, paging, export, and delete.", true),
		node("knowledge-base", "Knowledge base detail", "component", 1960, 510, "src/lib/components/workspace/Knowledge/KnowledgeBase.svelte", "Knowledge base files, text content, reindex/update/remove flows, and access control."),
		node("workspace-prompts", "Workspace prompts", "screen", 1600, 790, "src/routes/(app)/workspace/prompts/+page.svelte", "Prompt library with command search, import/export, clone, share, copy, history, tag filtering, and delete.", true),
		node("prompt-editor", "Prompt editor", "component", 1960, 770, "src/lib/components/workspace/Prompts/PromptEditor.svelte", "Create/edit versioned prompt content and metadata."),
		node("workspace-skills", "Workspace skills", "screen", 1600, 2350, "src/routes/(app)/workspace/skills/+page.svelte", "Skill list and management surface.", true),
		node("skill-editor", "Skill editor", "component", 1960, 2350, "src/lib/components/workspace/Skills/SkillEditor.svelte", "Create/edit skills and access state."),
		node("workspace-tools", "Workspace tools", "screen", 1600, 2570, "src/routes/(app)/workspace/tools/+page.svelte", "Tool list with import/export, manifest, valves, clone/share/delete, and filtering.", true),
		node("toolkit-editor", "Toolkit editor", "component", 1960, 2570, "src/lib/components/workspace/Tools/ToolkitEditor.svelte", "Create/edit toolkits, manifest, valves, and access control."),
		node("api-models", "Models API", "api", 2320, 350, "src/lib/apis/models/index.ts", "Workspace model item CRUD, tags, import, base models, access grants, toggle, and delete all."),
		node("api-knowledge", "Knowledge API", "api", 2320, 570, "src/lib/apis/knowledge/index.ts", "Knowledge base CRUD, search, file add/update/remove, reset, reindex, export, and access grants."),
		node("api-prompts", "Prompts API", "api", 2320, 790, "src/lib/apis/prompts/index.ts", "Prompt CRUD, search/list, tags, metadata, production version, history, diff, restore, toggle, and grants."),
		node("api-skills", "Skills API", "api", 2320, 2350, "src/lib/apis/skills/index.ts", "Skill CRUD, list/search, import/export, access grants, toggle, and delete."),
		node("api-tools", "Tools API", "api", 2320, 2570, "src/lib/apis/tools/index.ts", "Tool CRUD, load by URL, import/export, access grants, valves specs/values, user valves, and delete."),
		node("admin-layout", "Admin layout", "screen", 1240, 2720, "src/routes/(app)/admin/+layout.svelte", "Admin-only shell with tab navigation for users, analytics, evaluations, functions, and settings."),
		node("admin-users", "Admin users", "screen", 1600, 2820, "src/routes/(app)/admin/users/+page.svelte", "Users/groups administration, user details, roles, default permissions, and user chats.", true),
		node("admin-analytics", "Admin analytics", "screen", 1600, 3040, "src/routes/(app)/admin/analytics/+page.svelte", "Analytics dashboards for models, users, token usage, model chats, daily stats, and summaries.", true),
		node("admin-evals", "Admin evaluations", "screen", 1960, 2820, "src/routes/(app)/admin/evaluations/+page.svelte", "Feedbacks, leaderboard, model activity chart, and evaluation config surfaces.", true),
		node("admin-functions", "Admin functions", "screen", 1960, 3040, "src/routes/(app)/admin/functions/+page.svelte", "Function inventory, create/edit, import/export, valves, global toggle, and delete.", true),
		node("admin-settings", "Admin settings", "screen", 2320, 2820, "src/routes/(app)/admin/settings/+page.svelte", "Admin settings tabs: general, interface, connections, models, documents, web search, code execution, images, audio, evaluations, pipelines, database, integrations.", true),
		node("api-users", "Users / groups APIs", "api", 2680, 2820, "src/lib/apis/users + groups", "User/group CRUD, roles, permissions, status, settings, info, and user group membership."),
		node("api-analytics", "Analytics API", "api", 2680, 3040, "src/lib/apis/analytics/index.ts", "Model/user analytics, messages, summaries, daily stats, token usage, model chats, and model overview."),
		node("api-evals", "Evaluations API", "api", 3040, 2820, "src/lib/apis/evaluations/index.ts", "Evaluation config, feedback items/export, leaderboard, model history, feedback model ids, and feedback CRUD."),
		node("api-functions", "Functions API", "api", 3040, 3040, "src/lib/apis/functions/index.ts", "Function CRUD, URL load, export, toggle, global toggle, valves specs/values, user valves, and delete."),
		node("api-configs", "Configs API", "api", 3400, 2820, "src/lib/apis/configs/index.ts", "Import/export config, connections, tool/terminal servers, OAuth clients, code execution, model defaults, banners, and verification."),
		node("notes", "Notes", "screen", 1240, 2140, "src/routes/(app)/notes/+page.svelte", "Notes list shell with sidebar/user menu and Notes component.", true),
		node("note-editor", "Note editor", "component", 1600, 2140, "src/lib/components/notes/NoteEditor.svelte", "Rich note editing plus note chat assistant, controls, record menu, and access flows."),
		node("api-notes", "Notes API", "api", 1960, 2140, "src/lib/apis/notes/index.ts", "Notes CRUD, search/list, pinned notes, access grants, and pin toggles."),
		node("channels", "Channels", "screen", 1240, 2350, "src/routes/(app)/channels/[id]/+page.svelte", "Channel conversation route for teams/DM-style messaging.", true),
		node("channel-component", "Channel component", "component", 1600, 2350, "src/lib/components/channel/Channel.svelte", "Channel navbar, messages, thread, info modal, pinned messages, webhooks, members, and message input."),
		node("api-channels", "Channels API", "api", 1960, 2350, "src/lib/apis/channels/index.ts", "Channel CRUD, members, messages, threads, reactions, pinning, webhooks, DM lookup, and deletes."),
		node("automations", "Automations", "screen", 1240, 2570, "src/routes/(app)/automations/+page.svelte", "Automation list with search, create/edit modal, enable toggle, run history, delete, and pagination.", true),
		node("automation-modal", "Automation modal", "component", 1600, 2570, "src/lib/components/AutomationModal.svelte", "Create/edit automation forms and scheduling/run configuration."),
		node("api-automations", "Automations API", "api", 1960, 2790, "src/lib/apis/automations/index.ts", "Automation list, create, get, update, toggle, run, delete, and run history."),
		node("calendar", "Calendar", "screen", 1240, 3040, "src/routes/(app)/calendar/+page.svelte", "Calendar screen with calendar selector, sidebar, event view, event modal, default calendar, and create calendar modal.", true),
		node("api-calendar", "Calendar API", "api", 1600, 3040, "src/lib/apis/calendar/index.ts", "Calendar CRUD, default calendar, events CRUD/search, and RSVP."),
		node("playground-layout", "Playground layout", "screen", 3040, 360, "src/routes/(app)/playground/+layout.svelte", "Admin-only playground shell with tabs."),
		node("playground-chat", "Playground chat", "screen", 3400, 250, "src/routes/(app)/playground/+page.svelte", "Playground chat route using playground Chat component.", true),
		node("playground-completions", "Playground completions", "screen", 3400, 470, "src/routes/(app)/playground/completions/+page.svelte", "Completions playground route.", true),
		node("playground-images", "Playground images", "screen", 3400, 690, "src/routes/(app)/playground/images/+page.svelte", "Images playground route.", true),
		node("api-images", "Images API", "api", 3760, 690, "src/lib/apis/images/index.ts", "Image config, verify URL, generation models, image generation, and edits."),
		node("screenshots", "Screenshot capture slots", "capture", 3760, 1120, "frontend-map/screenshots/", "Place real screenshots here after running Open WebUI locally. Each mapped screen can link to a matching PNG by route slug.", true)
	],
	edges: [
...
function node(id, title, type, x, y, path, description, screenshot = false) {
	return { id, title, type, x, y, w: 238, h: screenshot ? 150 : 126, path, description, screenshot };
}
...
const overviewGroups = [
	["App shell", ["root-layout", "auth", "app-layout", "sidebar", "navbar-menu", "search-modal", "settings-modal", "stores"]],
	[
		"Chat",
		[
			"chat-home",
...
document.getElementById("removeAnnotationButton").addEventListener("click", removeSelectedAnnotation);
...
function autoBalanceNodes(activeIds = activeNodeIds()) {
	...
	const activeEdges = mapData.edges.filter((item) => activeIds.has(item.from) && activeIds.has(item.to));
	...
	for (const layer of layers) {
		layer.sort((a, b) => domainRank(a) - domainRank(b) || a.y - b.y || a.title.localeCompare(b.title));
	}
	...
	const maxColumnHeight = Math.max(...columnHeights, layoutConfig.minSurfaceHeight - layoutConfig.paddingY * 2);
...
function runForceBalance(nodes, edges) {
	...
	const centers = new Map(nodes.map((item) => [item.id, { x: item.x + item.w / 2, y: item.y + item.h / 2 }]));
...
function domainRank(item) {
	...
	if (item.id.includes("chat") || item.id.includes("message") || item.id.includes("model-selector")) return 1;
	if (item.id.includes("workspace") || item.id.includes("model") || item.id.includes("knowledge") || item.id.includes("prompt") || item.id.includes("skill") || item.id.includes("tool")) return 2;
	if (item.id.includes("note") || item.id.includes("channel") || item.id.includes("automation") || item.id.includes("calendar")) return 3;
	if (item.id.includes("admin") || item.id.includes("eval") || item.id.includes("analytics") || item.id.includes("config") || item.id.includes("function")) return 4;
...
function renderNodes() {
	...
	for (const item of mapData.nodes) {
		...
		el.addEventListener("pointermove", (event) => {
			...
			nodeDrag.moved =
				nodeDrag.moved || Math.hypot(event.clientX - nodeDrag.startX, event.clientY - nodeDrag.startY) > 3;
			el.style.left = `${item.x}px`;
			el.style.top = `${item.y}px`;
...
function renderNodeList() {
	...
	const configuredOutNodes = mapData.nodes.filter((item) => !enabledTypes.has(item.type) && visible.has(item.id));
...
function renderStats() {
	...
	const visibleEdges = mapData.edges.filter((item) => visible.has(item.from) && visible.has(item.to));
	stats.innerHTML = `
...
function selectItem(kind, id, updateInspector = true) {
	...
	const item = kind === "node" ? nodeById.get(id) : mapData.edges.find((edgeItem) => edgeItem.id === id);
	if (!item) return;
...
function exportMarkdown() {
	...
	for (const item of mapData.nodes) {
		...
		if (!note) continue;
		lines.push(`### ${item.title}`, "", `- Type: ${item.type}`, `- Path: \`${item.path}\``, "", note, "");
	}
...
function visibleNodeIds() {
	...
	const ids = new Set();
	for (const item of mapData.nodes) {
		if ([item.title, item.type, item.path, item.description, item.id].some(textMatches)) ids.add(item.id);
	}
	for (const item of mapData.edges) {
